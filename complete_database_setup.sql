-- =========================================================
-- TATVISHA DMS COMPLETE UNIFIED DATABASE SETUP SCRIPT
...
-- =========================================================
-- SYSTEM SETUP: ENUMS AND BASIC RELATIONS
-- Ensure we create these before running migration fixes
-- =========================================================
...
DO $body$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'app_role') THEN
        CREATE TYPE public.app_role AS ENUM ('owner', 'admin', 'salesperson');
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'order_status') THEN
        CREATE TYPE public.order_status AS ENUM ('draft', 'pending_approval', 'approved', 'rejected', 'dispatched', 'delivered', 'cancelled');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'pack_type') THEN
        CREATE TYPE public.pack_type AS ENUM ('unit', 'packet', 'case', 'pouch', 'box', 'jar', 'bottle', 'tin', 'can', 'acb', 'sachet', 'kg');
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'payment_method') THEN
        CREATE TYPE public.payment_method AS ENUM ('cash', 'upi', 'cheque', 'bank_transfer', 'card', 'other');
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'movement_type') THEN
        CREATE TYPE public.movement_type AS ENUM ('grn_inbound', 'order_fulfillment', 'manual_adjustment', 'stock_transfer', 'reversal');
    END IF;
END $body$;
...
CREATE TABLE IF NOT EXISTS public.shops (
  ...
  shop_type public.shop_type NOT NULL DEFAULT 'basic',
  is_active BOOLEAN NOT NULL DEFAULT true,
  beat_route_id UUID,
...
CREATE TABLE IF NOT EXISTS public.products (
  ...
  brand TEXT DEFAULT 'Tatvisha Enterprises',
  ...
  -- Final table state columns (prevent downstream script ordering & update errors)
  ...
  preferred_sell_unit public.pack_type DEFAULT 'case',
  base_weight_unit TEXT DEFAULT 'g',
  weight_per_unit_grams NUMERIC(12,4),
  display_weight_unit TEXT DEFAULT 'g',
  division TEXT,
  sub_category TEXT,
  item_pack_type TEXT,
  case_type TEXT DEFAULT 'carton',
  case_qty_value NUMERIC,
  case_qty_unit TEXT,
  base_unit TEXT,
  is_chain_item BOOLEAN DEFAULT FALSE,
  is_mrp_priced BOOLEAN DEFAULT FALSE,
  chain_mrp_label TEXT,
  target_margin_basic NUMERIC DEFAULT 15,
  target_margin_bronze NUMERIC DEFAULT 10,
  target_margin_silver NUMERIC DEFAULT 7,
  target_margin_gold NUMERIC DEFAULT 5,
  target_margin_premium NUMERIC DEFAULT 3,
  batch_number TEXT,
  rbp_bag DECIMAL(10,2) DEFAULT 0,
  units_per_bag INTEGER DEFAULT 1,
  bag_invoice_price DECIMAL(10,2) DEFAULT 0,
  carton_invoice_price DECIMAL(10,2) DEFAULT 0,
  target_margin_chain DECIMAL(5,2) DEFAULT 10,
  min_price DECIMAL(10,2) NOT NULL DEFAULT 0,
  min_selling_price DECIMAL(10,2) DEFAULT 0,
  target_margin_retailer DECIMAL(5,2) DEFAULT 50,
  target_margin_wholesaler DECIMAL(5,2) DEFAULT 30,
  target_margin_distributor DECIMAL(5,2) DEFAULT 15,
  case_qty_value_legacy NUMERIC,
  case_qty_unit_legacy TEXT
);
CREATE INDEX IF NOT EXISTS idx_products_sku ON public.products(sku);
CREATE INDEX IF NOT EXISTS idx_products_name ON public.products(name);

-- 6. purchase_invoices Table
...
CREATE TABLE IF NOT EXISTS public.inventory_batches (
  ...
  freight_cost_per_unit NUMERIC(10,4) NOT NULL DEFAULT 0,
  handling_cost_per_unit NUMERIC(10,4) NOT NULL DEFAULT 0,
  warehouse_id UUID REFERENCES public.warehouses(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS public.order_items (
  ...
  line_total_tax_exclusive NUMERIC(15,2),
  line_tax_amount NUMERIC(15,2),
  pack_type public.pack_type NOT NULL DEFAULT 'unit',
...
CREATE TABLE IF NOT EXISTS public.invoices (
  ...
  type public.invoice_type NOT NULL DEFAULT 'gst',
  is_void BOOLEAN NOT NULL DEFAULT false,
  discount_amount NUMERIC(15,2) DEFAULT 0,
  created_by UUID,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
...
-- 12. product_price_tiers Table
CREATE TABLE IF NOT EXISTS public.product_price_tiers (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  product_id UUID NOT NULL REFERENCES public.products(id) ON DELETE CASCADE,
  shop_type public.shop_type NOT NULL DEFAULT 'basic',
  pack_type public.pack_type NOT NULL DEFAULT 'packet',
  price NUMERIC(10,2) NOT NULL DEFAULT 0,
  is_auto_calculated BOOLEAN DEFAULT false,
  source_landed_cost NUMERIC(10,2),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- 13. Schemes Table (Relational scheme management)
...
-- 14. Stock Movements Ledger Table (Audit trail)
CREATE TABLE IF NOT EXISTS public.stock_movements (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  product_id UUID NOT NULL REFERENCES public.products(id) ON DELETE CASCADE,
  batch_id UUID REFERENCES public.inventory_batches(id) ON DELETE SET NULL,
  warehouse_id UUID REFERENCES public.warehouses(id) ON DELETE SET NULL,
  quantity NUMERIC(12,2) NOT NULL, -- positive for inbound, negative for outbound
  movement_type public.movement_type NOT NULL,
  reference_id UUID, -- order_id, purchase_invoice_id, or stock_adjustment_id etc.
  notes TEXT,
...
-- 15. Order Lines Table (FIFO batch depletion tracked per line)
CREATE TABLE IF NOT EXISTS public.order_lines (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  order_id UUID NOT NULL REFERENCES public.orders(id) ON DELETE CASCADE,
  product_id UUID NOT NULL REFERENCES public.products(id) ON DELETE CASCADE,
  batch_id UUID REFERENCES public.inventory_batches(id) ON DELETE SET NULL, -- optionally batch-aware
  quantity NUMERIC(10,2) NOT NULL,
  unit_price NUMERIC(10,2) NOT NULL,
  gst_rate NUMERIC(5,2) NOT NULL DEFAULT 5,
  line_total NUMERIC(12,2) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_order_lines_batch_id ON public.order_lines(batch_id);

-- 16. Collections Table (Partial payments & multi-invoice settlements)
CREATE TABLE IF NOT EXISTS public.collections (
  ...
  reference TEXT,
  notes TEXT,
  invoice_id UUID REFERENCES public.invoices(id) ON DELETE SET NULL, -- optionally links to invoices
  collected_by UUID REFERENCES public.profiles(id),
  collected_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_collections_shop_id ON public.collections(shop_id);
CREATE INDEX IF NOT EXISTS idx_collections_invoice_id ON public.collections(invoice_id);


-- PHASE 0: INITIAL PRODUCTS TABLE DEFINITION
-- =========================================================

-- STEP 1: CREATE PRODUCTS TABLE
-- ===========================================
-- Note: Re-declared above in correct dependency order. Inline reference comments map to public.products.

-- ===========================================

-- =========================================================
-- PHASE 1: SEQUENTIAL DATABASE MIGRATIONS
-- =========================================================



-- ---------------------------------------------------------
-- Migration File: 20260429000000_fix_orders_profiles_fk.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260429150000_gap_fixes.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260429160000_pricing_enhancements.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260429170000_bag_pricing_and_margins.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260429180000_hierarchy_and_logic.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260429_landed_cost_v2.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260430000000_price_history.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260501000000_cleanup_redundant_tables.sql
-- ---------------------------------------------------------

-- Cleanup redundant and unused tables
DROP TABLE IF EXISTS public.inventory;
DROP TABLE IF EXISTS public.purchase_invoice_items;
DROP TABLE IF EXISTS public.notifications;
DROP TABLE IF EXISTS public.import_mapping_templates;
DROP TABLE IF EXISTS public.user_roles;


-- ---------------------------------------------------------
-- Migration File: 20260501000500_fix_rls_policies.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260501100000_rename_bag_carton_to_case.sql
-- ---------------------------------------------------------
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'packets_per_bag') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'packets_per_case') THEN
      ALTER TABLE public.products DROP COLUMN packets_per_bag;
    ELSE
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'units_per_bag') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'units_per_case') THEN
      ALTER TABLE public.products DROP COLUMN units_per_bag;
    ELSE
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'rbp_bag') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'rbp_case') THEN
      ALTER TABLE public.products DROP COLUMN rbp_bag;
    ELSE
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'bag_landed_price') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'case_landed_price') THEN
      ALTER TABLE public.products DROP COLUMN bag_landed_price;
    ELSE
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'bag_freight_cost') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'case_freight_cost') THEN
      ALTER TABLE public.products DROP COLUMN bag_freight_cost;
    ELSE
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'bag_invoice_price') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'case_invoice_price') THEN
      ALTER TABLE public.products DROP COLUMN bag_invoice_price;
    ELSE
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'units_per_carton') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'units_per_case_legacy') THEN
      ALTER TABLE public.products DROP COLUMN units_per_carton;
    ELSE
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'carton_invoice_price') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'case_invoice_price_legacy') THEN
      ALTER TABLE public.products DROP COLUMN carton_invoice_price;
    ELSE
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'unit_per_carton') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'units_per_case_v1') THEN
      ALTER TABLE public.products DROP COLUMN unit_per_carton;
    ELSE
...
-- ---------------------------------------------------------
-- Migration File: 20260501110000_update_shop_tiers.sql
-- ---------------------------------------------------------
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'target_margin_retailer') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'target_margin_silver') THEN
      ALTER TABLE public.products DROP COLUMN target_margin_retailer;
    ELSE
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'target_margin_wholesaler') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'target_margin_gold') THEN
      ALTER TABLE public.products DROP COLUMN target_margin_wholesaler;
    ELSE
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'target_margin_distributor') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'target_margin_premium') THEN
      ALTER TABLE public.products DROP COLUMN target_margin_distributor;
    ELSE
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'target_margin_chain') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'target_margin_bronze') THEN
      ALTER TABLE public.products DROP COLUMN target_margin_chain;
    ELSE
...
-- ---------------------------------------------------------
-- Migration File: 20260501120000_update_product_defaults.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260501130000_ensure_inventory_table.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260501150000_fix_enum_transaction.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260501160000_ensure_product_columns.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260502020000_final_schema_fix.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260502030000_fix_order_items_fk.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260502040000_add_product_batch_number.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260502050000_update_invoice_payment_totals.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260502060000_add_shop_id_to_invoices.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260502070000_cleanup_product_columns.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260502080000_remove_remaining_price_columns_and_fix_trigger.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260502100000_fix_margin_report_view.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260502110000_audit_trail_complete.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260502120000_prepare_enums_and_cleanup.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260502120010_apply_data_consistency.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260503000000_grn_enhancements.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260503000100_stock_management_v2.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260503050000_update_vermicelli_names.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260503100000_add_missing_columns.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504000000_stock_engine_v1.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504050000_import_engine.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504100000_enhanced_product_logic.sql
-- ---------------------------------------------------------
  ...
  FOR v_row IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
    BEGIN
      ...
      INSERT INTO public.products (
        ...
        (v_row->>'pack_size_value')::NUMERIC,
        v_row->>'pack_size_unit',
        COALESCE(v_row->>'brand', 'Tatvisha Enterprises'),
        (v_row->>'units_per_packet')::INT,
        (v_row->>'packets_per_case')::INT,
...
-- ---------------------------------------------------------
-- Migration File: 20260504100000_product_bulk_import_schema.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504100001_advanced_stock_conversion.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504100002_recompute_inventory_rpc.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504100003_fix_import_rpc_casting.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504100004_enhanced_product_logic.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504100005_product_bulk_import_schema.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504100009_final_schema_reconciliation.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504110000_stock_calculation_engine.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504110200_add_kg_to_pack_type.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504111200_handle_kg_conversion.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504113000_case_qty_and_conversions.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504120000_final_import_sync.sql
-- ---------------------------------------------------------
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'brand') THEN
    ALTER TABLE public.products ADD COLUMN brand TEXT DEFAULT 'Tatvisha Enterprises';
  END IF;
...
-- ---------------------------------------------------------
-- Migration File: 20260504121000_enhanced_product_logic_final.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504121500_final_import_business_rules.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260504123000_update_existing_products_logic.sql
-- ---------------------------------------------------------
...
BEGIN
  ...
  UPDATE public.products
  SET brand = 'Tatvisha Enterprises'
...
-- ---------------------------------------------------------
-- Migration File: 20260504124500_product_data_repair.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260505000000_update_import_rpc.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260505193000_fix_order_items_pack_type.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260505194000_ensure_admin_delete_rights.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260505200000_fix_deletion_rls.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506000000_add_backdating.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506010000_master_reconciliation.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506020000_profiles_security.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506030000_fix_purchase_invoices_rls.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506120000_ensure_price_tier_unique.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506130000_unify_grn_rls.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506140000_standardize_enums.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506150000_fix_inventory_sync.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506160000_fix_orders_columns.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506160000_fix_profiles_rls_and_admin.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506160005_fix_profiles_rls_and_admin.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506170000_robust_stock_sync_rpc.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506180000_final_inventory_fix.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506190000_final_definitive_fix.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506200000_emergency_stock_repair.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506210000_nuclear_stock_rebuild.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506220000_fix_shops_rls.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506230000_fix_payments_rls.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506240000_update_dispatch_rpc.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260506250000_ensure_core_relations.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260507000000_fix_conversions_final.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260507000000_unit_type_system.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260507000005_unit_type_system.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260507010000_standardize_sell_units.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260507020000_fix_case_deduction.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260507020000_fix_price_history_profiles_fk.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260507020001_fix_all_profiles_joins_fk.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260507020005_fix_price_history_profiles_fk.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508000000_definitive_inventory_v2.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508000001_manual_inventory_reset.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508001000_fix_stale_triggers.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508002000_definitive_repair.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508003000_atomic_recovery.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508004000_restore_avg_landed_cost.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508004000_stock_transfer_ledger.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508004005_stock_transfer_ledger.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508005000_fix_conversion_overloads.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508005001_fix_v_product_stock.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508006000_auto_toggle_active.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508006000_manual_batch_ledger.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508006001_manual_batch_ledger.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508006005_manual_batch_ledger.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508007000_definitive_enum_fix.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508008000_add_order_discount.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508009000_revert_order_rpc.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508010000_warehouse_management.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508015000_inventory_sync_final_fix.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508016000_warehousing_trigger_fix.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508017000_fix_inventory_primary_key.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508018000_ultimate_inventory_fix.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508019000_force_inventory_pk_v2.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508020000_performance_indexes.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508021000_dashboard_optimized.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508023000_nuclear_inventory_pk_fix.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508024000_warehouse_aware_sync.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508030000_fix_inventory_constraints.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508031000_advanced_packing_healer.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508032000_stock_transaction_view.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508103500_stock_audit_system.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260508110000_stock_ledger_healing.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260509010000_assign_owner_roles.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260509020000_margin_and_gst_fix.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260509030000_invoice_and_notifications.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260509040000_atomic_stock_management.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260509050000_warehouse_aware_dispatch.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260509060000_optimize_views_and_freight.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260509070000_allow_profile_insert.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260509080000_assign_dibya_owner.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260509090000_fix_ownership_and_rls.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260509100000_absolute_ownership_fix.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260509110000_repair_profiles_rls.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260509120000_fix_rls_recursion_final.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260509130000_break_recursion_completely.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260509140000_fix_profiles_rls_final.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260510030000_data_flow_fixes.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260510035500_pricing_standardization_fixes.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260510040000_void_invoices.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260510045000_pricing_and_stock_fixes.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260510050000_audit_logic_fixes.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260510143100_pin_auth_system.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511000000_fix_stock_views_and_reversals.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511001000_fix_order_nan_and_pricing_fallbacks.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511002000_fix_kg_option_columns.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511003000_perf_and_consistency.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511110000_auto_profile_trigger.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511120000_profiles_admin_update.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511130000_refresh_pin_function.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511140000_ensure_pin_tables.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511150000_fix_get_salesperson_list.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511160000_force_refresh_rpc.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511170000_rename_rpc_for_cache.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511180000_rename_verify_rpc.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511190000_fix_orders_is_void.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511193000_assign_dibya2_owner.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511200000_fix_dibya2_salesperson.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511210000_ensure_orders_is_void_column.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511220000_emergency_schema_repair.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511230000_rpc_overloads.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511240000_definitive_rls_cleanup.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511250000_definitive_rpc_fix.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511260000_secure_order_insertion.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511270000_fix_pin_verification_logic.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511280000_business_logic_integrity.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511290000_stabilize_auth_rls.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511300000_hardened_auth_and_home.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511400000_user_repair_and_perf.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511600000_definitive_schema_and_users.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511700000_recursion_killer.sql
-- ---------------------------------------------------------
...
CREATE OR REPLACE FUNCTION public.fix_table_rls(t_name text) RETURNS void AS $body$
DECLARE
    pol record;
BEGIN
...
-- ---------------------------------------------------------
-- Migration File: 20260511800000_user_definitive_auth_fix.sql
-- ---------------------------------------------------------

-- MIGRATION: 20260511800000_user_definitive_auth_fix.sql
-- BHARAT MASALA — DEFINITIVE AUTH FIX
...
-- 5. Set correct roles for BOTH users
...
AS $body$ BEGIN RETURN public.verify_staff_pin_v2(p_profile_id, p_pin); END; $body$;
...
-- ---------------------------------------------------------
-- Migration File: 20260511900000_definitive_auth_fix_v2.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511900001_get_auth_data_rpc.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511900002_fix_rls_recursion_definitive.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260511900003_auth_performance_indexes.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260512000000_inventory_movement_engine.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260512000001_inventory_batch_view.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260512060000_fortress_auth_v3.sql
-- ---------------------------------------------------------
...
AS $body$ SELECT * FROM public.get_salesperson_list(); $body$;
...
-- ---------------------------------------------------------
-- Migration File: 20260513000000_summary_tables.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260513010000_dashboard_optimized_v2.sql
-- ---------------------------------------------------------

-- MIGRATION: DASHBOARD-OPTIMIZATION-V2
-- Purpose: Update get_dashboard_stats to use summary tables for faster execution.
...
DECLARE
    ...
    pending_queue_json JSON;
BEGIN
    -- Counts that are relatively cheap
    ...
    -- Metrics from summary_daily_performance
    -- Note: Summary tables are currently global. For warehouse-specific view, we might need a separate table or query raw orders for specific metrics.
    -- For now, if p_warehouse_id is provided, we query raw orders for today's metrics to be accurate.
    IF p_warehouse_id IS NULL THEN
        SELECT 
            COALESCE(revenue, 0), 
            COALESCE(delivered_count, 0)
        INTO sales_today_val, delivered_today_count 
        FROM public.summary_daily_performance 
    ...
    -- Financial Outstanding (Filter by warehouse if provided)
    IF p_warehouse_id IS NULL THEN
        SELECT COALESCE((val_json->>'total_outstanding')::numeric, 0) INTO outstanding_val 
        FROM public.summary_global_stats 
        WHERE key = 'financial_summary';
    ELSE
    ...
    -- Low Stock Items (Filter by warehouse if provided)
    SELECT json_agg(t) INTO low_stock_json FROM (
        SELECT id, name, min_stock, stock_base_units as quantity, units_per_packet, pack_size_value, pack_size_unit
        FROM public.v_product_stock_warehouse
        WHERE is_active = true 
        AND stock_base_units <= min_stock
        AND (p_warehouse_id IS NULL OR warehouse_id = p_warehouse_id)
        ORDER BY (stock_base_units / NULLIF(min_stock, 0)) ASC
        LIMIT 5
    ) t;

    -- Expiring Batches (Next 30 days, Filter by warehouse)
    SELECT json_agg(t) INTO expiring_json FROM (
        SELECT b.id, b.batch_number, b.expiry_date, b.remaining_qty, b.product_id, p.name as product_name
        FROM public.inventory_batches b
        JOIN public.products p ON b.product_id = p.id
    ...
    -- Top Shops Performance (Rolling Month, Dispatched + Delivered)
    SELECT json_agg(t) INTO top_shops_json FROM (
        SELECT s.id as shop_id, s.name, sum(o.total) as total
        FROM public.orders o
        JOIN public.shops s ON o.shop_id = s.id
        WHERE o.status IN ('delivered', 'dispatched') 
        AND (o.delivered_at >= date_trunc('month', CURRENT_DATE) OR o.dispatched_at >= date_trunc('month', CURRENT_DATE))
        AND o.is_void = false
    ...
    -- Top Salespeople Performance (Rolling Month, Dispatched + Delivered)
    SELECT json_agg(t) INTO top_salespeople_json FROM (
        SELECT pr.full_name as name, sum(o.total) as total
        FROM public.orders o
        JOIN public.profiles pr ON o.salesperson_id = pr.id
        WHERE o.status IN ('delivered', 'dispatched') 
        AND (o.delivered_at >= date_trunc('month', CURRENT_DATE) OR o.dispatched_at >= date_trunc('month', CURRENT_DATE))
        AND o.is_void = false
        AND (p_warehouse_id IS NULL OR o.warehouse_id = p_warehouse_id)
    ...
    IF p_warehouse_id IS NULL THEN
        SELECT json_agg(t) INTO trend_json FROM (
            ...
            FROM generate_series(CURRENT_DATE - INTERVAL '6 days', CURRENT_DATE, INTERVAL '1 day') AS d(date)
            LEFT JOIN public.summary_daily_performance s ON s.date = d.date::date
            ORDER BY d.date ASC
        ) t;
    ELSE
        SELECT json_agg(t) INTO trend_json FROM (
    ...
    -- Pending Approval Queue Details
    SELECT json_agg(t) INTO pending_queue_json FROM (
        SELECT 
            o.id, 
            o.order_number, 
        ...
        JOIN public.shops s ON o.shop_id = s.id
        LEFT JOIN public.profiles pr ON o.salesperson_id = pr.id
        WHERE o.status = 'pending_approval' AND o.is_void = false
        AND (p_warehouse_id IS NULL OR o.warehouse_id = p_warehouse_id)
        ORDER BY o.created_at ASC
        LIMIT 5
    ) t;
...
-- ---------------------------------------------------------
-- Migration File: 20260513020000_order_profitability_view.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260513020000_summary_tables_rls.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260513030000_expanded_order_invoice_views.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260513040000_fix_revert_rpc_security.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260513100000_heal_data_views.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260514000000_atomic_draft_save.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260514000001_fix_summary_trigger_order_date.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260514010000_dashboard_staleness_fix.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260515000000_fix_views.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260516000000_fix_revert_permissions.sql
-- ---------------------------------------------------------

-- Fix revert_order_to_approved to be SECURITY DEFINER
-- This ensures that it can update inventory_batches and order_batch_deductions even when called by non-admin users (who have the permission to trigger it via UI)
...
BEGIN
    ...
    UPDATE public.orders SET status = 'approved', dispatched_at = NULL, delivered_at = NULL WHERE id = p_order_id;
    UPDATE public.invoices SET is_void = true WHERE order_id = p_order_id;
...
-- Grant access
GRANT EXECUTE ON FUNCTION public.revert_order_to_approved(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.revert_order_to_approved(UUID) TO anon;


-- ---------------------------------------------------------
-- Migration File: 20260516010000_fix_staff_list_perms.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260517000000_summary_tables_security_definer.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260517001000_definitive_function_fix.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260517002000_fix_summary_tables_date.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260517003000_add_warehouse_to_profiles.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260518000000_unify_roles_storage.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260518000001_fix_rpcs_for_unified_roles.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260518000002_cleanup_rpcs_after_unification.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260520000000_fix_v_product_stock_warehouse.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260520000001_fix_draft_items_and_batches.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260520000002_fix_draft_v3.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260521000000_protect_order_status_on_edit.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260521000001_fix_invoice_deduction_audit_trail.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260521000002_fix_anonymous_invoices.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260522000000_fix_revert_and_owner_role.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260523000000_ledger_integrity_sync.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260525000000_heal_product_columns.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260525000000_wac_engine.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260526000000_global_settings.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260527000000_fix_all_pin_rpcs.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260527000001_fix_salesman_permissions.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260527000002_fix_warehouse_view_and_defaults.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260527000003_enhanced_dashboard_stats.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260528000000_fix_invoice_deduction_conversion.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260529000000_definitive_order_revert_permissions.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260530000000_fix_order_revert_reconciliation.sql
-- ---------------------------------------------------------

-- Migration: 20260530000000_fix_order_revert_reconciliation.sql
-- Goal: Fix column nonexistent error inside order reversal by utilizing public.record_inventory_movement
...
-- ---------------------------------------------------------
-- Migration File: 20260531000000_allow_delete_stock_audits.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260531010000_fix_kg_g_packet_conversion.sql
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- Migration File: 20260601000000_fix_revert_joins_and_error_toasts.sql
-- ---------------------------------------------------------
...
INSERT INTO products (
  ...
  ('Turmeric Powder (Haldi) [5kg]', 'BM-TURMERIC-5KG-BAG', '09103030', 2060, 196, 4116, 4, 5, 'BASIC SPICES', 'BAG', 5.0, 'kg', 'BAG', 196, 'Tatvisha Enterprises', true, 0),
  ('Turmeric Powder (Haldi) [1 Kg]', 'BM-TURMERIC-1KG-BAG', '09103030', 416, 198, 3326, 16, 5, 'BASIC SPICES', 'BAG', 1.0, 'kg', 'BAG', 198, 'Tatvisha Enterprises', true, 0),
  ('Turmeric Powder (Haldi) [500 gms]', 'BM-TURMERIC-500GM-BAG', '09103030', 210, 200, 3360, 32, 5, 'BASIC SPICES', 'BAG', 500.0, 'gms', 'BAG', 200, 'Tatvisha Enterprises', true, 0),
  ('Turmeric Powder (Haldi) [250 gms]', 'BM-TURMERIC-250GM-BAG', '09103030', 106, 202, 3394, 64, 5, 'BASIC SPICES', 'BAG', 250.0, 'gms', 'BAG', 202, 'Tatvisha Enterprises', true, 0),
  ('Turmeric Powder (Haldi) [200 gms]', 'BM-TURMERIC-200GM-BAG', '09103030', 85, 203, 3410, 80, 5, 'BASIC SPICES', 'BAG', 200.0, 'gms', 'BAG', 203, 'Tatvisha Enterprises', true, 0),
  ('Turmeric Powder (Haldi) [100 gms]', 'BM-TURMERIC-100GM-BAG', '09103030', 43, 205, 3444, 160, 5, 'BASIC SPICES', 'BAG', 100.0, 'gms', 'BAG', 205, 'Tatvisha Enterprises', true, 0),
  ('Turmeric Powder (Haldi) [50 gms]', 'BM-TURMERIC-50GMS-BAG', '09103030', 22, 209, 3511, 320, 5, 'BASIC SPICES', 'BAG', 50.0, 'gms', 'BAG', 209, 'Tatvisha Enterprises', true, 0),
  ('Turmeric Powder Jar (Haldi) [1 Kg]', 'BM-TURMERIC-1KG-JAR', '09103030', 488, 232, 2923, 12, 5, 'BASIC SPICES', 'JAR', 1.0, 'kg', 'BAG', 232, 'Tatvisha Enterprises', true, 0),
  ('Turmeric Powder Jar (Haldi) [500 gms]', 'BM-TURMERIC-500GM-JAR', '09103030', 252, 240, 3024, 24, 5, 'BASIC SPICES', 'JAR', 500.0, 'gms', 'BAG', 240, 'Tatvisha Enterprises', true, 0),
  ('Turmeric Powder Jar (Haldi) [250 gms]', 'BM-TURMERIC-250GM-JAR', '09103030', 131, 249, 3137, 48, 5, 'BASIC SPICES', 'JAR', 250.0, 'gms', 'BAG', 249, 'Tatvisha Enterprises', true, 0),
  ('Turmeric Powder Jar (Haldi) [100 gms]', 'BM-TURMERIC-100GM-JAR', '09103030', 55, 261, 3289, 120, 5, 'BASIC SPICES', 'JAR', 100.0, 'gms', 'BAG', 261, 'Tatvisha Enterprises', true, 0),
  ('Chilli Powder [5kg]', 'BM-CHILLIPO-5KG-BAG', '09042211', 2540, 242, 5082, 4, 5, 'BASIC SPICES', 'BAG', 5.0, 'kg', 'BAG', 242, 'Tatvisha Enterprises', true, 0),
  ('Chilli Powder [1 kg]', 'BM-CHILLIPO-1KG-BAG', '09042211', 512, 244, 4099, 16, 5, 'BASIC SPICES', 'BAG', 1.0, 'kg', 'BAG', 244, 'Tatvisha Enterprises', true, 0),
  ('Chilli Powder [500 gms]', 'BM-CHILLIPO-500GM-BAG', '09042211', 258, 246, 4133, 32, 5, 'BASIC SPICES', 'BAG', 500.0, 'gms', 'BAG', 246, 'Tatvisha Enterprises', true, 0),
  ('Chilli Powder [250 gms]', 'BM-CHILLIPO-250GM-BAG', '09042211', 130, 248, 4166, 64, 5, 'BASIC SPICES', 'BAG', 250.0, 'gms', 'BAG', 248, 'Tatvisha Enterprises', true, 0),
  ('Chilli Powder [200 gms]', 'BM-CHILLIPO-200GM-BAG', '09042211', 105, 249, 4183, 80, 5, 'BASIC SPICES', 'BAG', 200.0, 'gms', 'BAG', 249, 'Tatvisha Enterprises', true, 0),
  ('Chilli Powder [100 gms]', 'BM-CHILLIPO-100GM-BAG', '09042211', 53, 250, 4200, 160, 5, 'BASIC SPICES', 'BAG', 100.0, 'gms', 'BAG', 250, 'Tatvisha Enterprises', true, 0),
  ('Chilli Powder [50 gms]', 'BM-CHILLIPO-50GMS-BAG', '09042211', 27, 255, 4284, 320, 5, 'BASIC SPICES', 'BAG', 50.0, 'gms', 'BAG', 255, 'Tatvisha Enterprises', true, 0),
  ('Chilli Powder Jar [250 gms]', 'BM-CHILLIPO-250GM-JAR', '09042211', 130, 248, 3125, 48, 5, 'BASIC SPICES', 'JAR', 250.0, 'gms', 'BAG', 248, 'Tatvisha Enterprises', true, 0),
  ('Chilli Powder Jar [100 gms]', 'BM-CHILLIPO-100GM-JAR', '09042211', 53, 250, 3150, 120, 5, 'BASIC SPICES', 'JAR', 100.0, 'gms', 'BAG', 250, 'Tatvisha Enterprises', true, 0),
  ('Coriander Powder (Dhania) [5kg]', 'BM-CORIANDE-5KG-BAG', '09092200', 1910, 182, 3822, 4, 5, 'BASIC SPICES', 'BAG', 5.0, 'kg', 'BAG', 182, 'Tatvisha Enterprises', true, 0),
  ('Coriander Powder (Dhania) [1 kg]', 'BM-CORIANDE-1KG-BAG', '09092200', 386, 184, 3091, 16, 5, 'BASIC SPICES', 'BAG', 1.0, 'kg', 'BAG', 184, 'Tatvisha Enterprises', true, 0),
  ('Coriander Powder (Dhania) [500 gms]', 'BM-CORIANDE-500GM-BAG', '09092200', 195, 186, 3125, 32, 5, 'BASIC SPICES', 'BAG', 500.0, 'gms', 'BAG', 186, 'Tatvisha Enterprises', true, 0),
  ('Coriander Powder (Dhania) [250 gms]', 'BM-CORIANDE-250GM-BAG', '09092200', 99, 188, 3158, 64, 5, 'BASIC SPICES', 'BAG', 250.0, 'gms', 'BAG', 188, 'Tatvisha Enterprises', true, 0),
  ('Coriander Powder (Dhania) [200 gms]', 'BM-CORIANDE-200GM-BAG', '09092200', 79, 189, 3175, 80, 5, 'BASIC SPICES', 'BAG', 200.0, 'gms', 'BAG', 189, 'Tatvisha Enterprises', true, 0),
  ('Coriander Powder (Dhania) [100 gms]', 'BM-CORIANDE-100GM-BAG', '09092200', 40, 190, 3192, 160, 5, 'BASIC SPICES', 'BAG', 100.0, 'gms', 'BAG', 190, 'Tatvisha Enterprises', true, 0),
  ('Coriander Powder (Dhania) [50 gms]', 'BM-CORIANDE-50GMS-BAG', '09092200', 21, 195, 3276, 320, 5, 'BASIC SPICES', 'BAG', 50.0, 'gms', 'BAG', 195, 'Tatvisha Enterprises', true, 0),
  ('Cumin Powder (Jeera) [500 gms]', 'BM-CUMINPOW-500GM-BAG', '09093200', 384, 366, 6149, 32, 5, 'BASIC SPICES', 'BAG', 500.0, 'gms', 'BAG', 366, 'Tatvisha Enterprises', true, 0),
  ('Cumin Powder (Jeera) [250 gms]', 'BM-CUMINPOW-250GM-BAG', '09093200', 193, 368, 6182, 64, 5, 'BASIC SPICES', 'BAG', 250.0, 'gms', 'BAG', 368, 'Tatvisha Enterprises', true, 0),
  ('Cumin Powder (Jeera) [200 gms]', 'BM-CUMINPOW-200GM-BAG', '09093200', 155, 369, 6199, 80, 5, 'BASIC SPICES', 'BAG', 200.0, 'gms', 'BAG', 369, 'Tatvisha Enterprises', true, 0),
  ('Cumin Powder (Jeera) [100 gms]', 'BM-CUMINPOW-100GM-BAG', '09093200', 78, 370, 6216, 160, 5, 'BASIC SPICES', 'BAG', 100.0, 'gms', 'BAG', 370, 'Tatvisha Enterprises', true, 0),
  ('Cumin Powder (Jeera) [50 gms]', 'BM-CUMINPOW-50GMS-BAG', '09093200', 39, 375, 6300, 320, 5, 'BASIC SPICES', 'BAG', 50.0, 'gms', 'BAG', 375, 'Tatvisha Enterprises', true, 0),
  ('Cinnamon Powder (Dalchin) [500 gms]', 'BM-CINNAMON-500GM-POU', '09061910', 465, 443, 3721, 16, 5, 'BASIC SPICES', 'POUCH', 500.0, 'gms', 'BAG', 443, 'Tatvisha Enterprises', true, 0),
  ('Cinnamon Powder (Dalchin) [100 gms]', 'BM-CINNAMON-100GM-POU', '09061910', 95, 453, 7610, 160, 5, 'BASIC SPICES', 'POUCH', 100.0, 'gms', 'BAG', 453, 'Tatvisha Enterprises', true, 0),
  ('Cinnamon Powder (Dalchin) [50 gms]', 'BM-CINNAMON-50GMS-POU', '09061910', 48, 455, 7644, 320, 5, 'BASIC SPICES', 'POUCH', 50.0, 'gms', 'BAG', 455, 'Tatvisha Enterprises', true, 0),
  ('Black Pepper Powder [50 gms]', 'BM-BLACKPEP-50GMS-POU', '09041200', 113, 1070, 8988, 160, 5, 'BASIC SPICES', 'POUCH', 50.0, 'gms', 'BAG', 1070, 'Tatvisha Enterprises', true, 0),
  ('Black Pepper Powder [100 gms]', 'BM-BLACKPEP-100GM-POU', '09042211', 224, 1065, 13419, 120, 5, 'BASIC SPICES', 'POUCH', 100.0, 'gms', 'BAG', 1065, 'Tatvisha Enterprises', true, 0),
  ('Black Pepper Powder [500 gms]', 'BM-BLACKPEP-500GM-POU', '09042211', 1103, 1050, 8820, 16, 5, 'BASIC SPICES', 'POUCH', 500.0, 'gms', 'BAG', 1050, 'Tatvisha Enterprises', true, 0),
  ('Kashmiri Chilli (ACB) [100 gms]', 'BM-KASHMIRI-100GM-ACB', '09042211', 177, 843, 10622, 120, 5, 'BASIC SPICES', 'ACB', 100.0, 'gms', 'BAG', 843, 'Tatvisha Enterprises', true, 0),
  ('Kashmiri Chilli (ACB) [50 gms]', 'BM-KASHMIRI-50GMS-ACB', '09042211', 90, 852, 7157, 160, 5, 'BASIC SPICES', 'ACB', 50.0, 'gms', 'BAG', 852, 'Tatvisha Enterprises', true, 0),
  ('Tomato Powder [100 gms]', 'BM-TOMATOPO-100GM-POU', '33012990', 130, 619, 7799, 120, 5, 'BASIC SPICES', 'POUCH', 100.0, 'gms', 'BAG', 619, 'Tatvisha Enterprises', true, 0),
  ('Tomato Powder [50 gms]', 'BM-TOMATOPO-50GMS-POU', '33012990', 66, 630, 5292, 160, 5, 'BASIC SPICES', 'POUCH', 50.0, 'gms', 'BAG', 630, 'Tatvisha Enterprises', true, 0),
  ('Ginger Powder [100 gms]', 'BM-GINGERPO-100GM-POU', '09101210', 81, 388, 4889, 120, 5, 'BASIC SPICES', 'POUCH', 100.0, 'gms', 'BAG', 388, 'Tatvisha Enterprises', true, 0),
  ('Ginger Powder [50 gms]', 'BM-GINGERPO-50GMS-POU', '09101210', 42, 398, 3343, 160, 5, 'BASIC SPICES', 'POUCH', 50.0, 'gms', 'BAG', 398, 'Tatvisha Enterprises', true, 0),
  ('Garlic Powder [100 gms]', 'BM-GARLICPO-100GM-POU', '07129020', 59, 282, 3553, 120, 5, 'BASIC SPICES', 'POUCH', 100.0, 'gms', 'BAG', 282, 'Tatvisha Enterprises', true, 0),
  ('Garlic Powder [50 gms]', 'BM-GARLICPO-50GMS-POU', '07129020', 31, 292, 2453, 160, 5, 'BASIC SPICES', 'POUCH', 50.0, 'gms', 'BAG', 292, 'Tatvisha Enterprises', true, 0),
  ('Curry Powder [100 gms]', 'BM-CURRYPOW-100GM-POU', '09109100', 99, 470, 5922, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 470, 'Tatvisha Enterprises', true, 0),
  ('Curry Powder [50 gms]', 'BM-CURRYPOW-50GMS-POU', '09109100', 50, 479, 4024, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 479, 'Tatvisha Enterprises', true, 0),
  ('Curry Powder Jar [100 gms]', 'BM-CURRYPOW-100GM-JAR', '09109100', 99, 470, 5922, 120, 5, 'BLENDED SPICES', 'JAR', 100.0, 'gms', 'CARTOON', 470, 'Tatvisha Enterprises', true, 0),
  ('Curry Powder Jar [250 gms]', 'BM-CURRYPOW-250GM-JAR', '09109100', 262, 499, 6287, 48, 5, 'BLENDED SPICES', 'JAR', 250.0, 'gms', 'CARTOON', 499, 'Tatvisha Enterprises', true, 0),
  ('Curry Powder Pouch [500 gms]', 'BM-CURRYPOW-500GM-POU', '09109100', 484, 461, 3872, 16, 5, 'BLENDED SPICES', 'POUCH', 500.0, 'gms', 'CARTOON', 461, 'Tatvisha Enterprises', true, 0),
  ('Meat Masala [100 gms]', 'BM-MEATMASA-100GM-POU', '09109100', 115, 549, 6917, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 549, 'Tatvisha Enterprises', true, 0),
  ('Meat Masala [50 gms]', 'BM-MEATMASA-50GMS-POU', '09109100', 59, 557, 4679, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 557, 'Tatvisha Enterprises', true, 0),
  ('Meat Masala Jar [100 gms]', 'BM-MEATMASA-100GM-JAR', '09109100', 115, 549, 6917, 120, 5, 'BLENDED SPICES', 'JAR', 100.0, 'gms', 'CARTOON', 549, 'Tatvisha Enterprises', true, 0),
  ('Meat Masala Jar [250 gms]', 'BM-MEATMASA-250GM-JAR', '09109100', 288, 548, 6905, 48, 5, 'BLENDED SPICES', 'JAR', 250.0, 'gms', 'CARTOON', 548, 'Tatvisha Enterprises', true, 0),
  ('Meat Masala Pouch [200 gms]', 'BM-MEATMASA-200GM-POU', '09109100', 188, 449, 3772, 40, 5, 'BLENDED SPICES', 'POUCH', 200.0, 'gms', 'CARTOON', 449, 'Tatvisha Enterprises', true, 0),
  ('Chicken Masala [100 gms]', 'BM-CHICKENM-100GM-POU', '09109100', 115, 549, 6917, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 549, 'Tatvisha Enterprises', true, 0),
  ('Chicken Masala [50 gms]', 'BM-CHICKENM-50GMS-POU', '09109100', 59, 557, 4679, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 557, 'Tatvisha Enterprises', true, 0),
  ('Chicken Masala Jar [100 gms]', 'BM-CHICKENM-100GM-JAR', '09109100', 115, 549, 6917, 120, 5, 'BLENDED SPICES', 'JAR', 100.0, 'gms', 'CARTOON', 549, 'Tatvisha Enterprises', true, 0),
  ('Chicken Masala Jar [250 gms]', 'BM-CHICKENM-250GM-JAR', '09109100', 288, 548, 6905, 48, 5, 'BLENDED SPICES', 'JAR', 250.0, 'gms', 'CARTOON', 548, 'Tatvisha Enterprises', true, 0),
  ('Garam Masala Super (ACB) [50 gms]', 'BM-GARAMMAS-50GMS-ACB', '09109100', 51, 481, 4040, 160, 5, 'BLENDED SPICES', 'ACB', 50.0, 'gms', 'CARTOON', 481, 'Tatvisha Enterprises', true, 0),
  ('Garam Masala Super (ACB) [500 gms]', 'BM-GARAMMAS-500GM-ACB', '09109100', 482, 459, 3856, 16, 5, 'BLENDED SPICES', 'ACB', 500.0, 'gms', 'CARTOON', 459, 'Tatvisha Enterprises', true, 0),
  ('Garam Masala [100 gms]', 'BM-GARAMMAS-100GM-POU', '09109100', 158, 753, 9488, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 753, 'Tatvisha Enterprises', true, 0),
  ('Garam Masala [50 gms]', 'BM-GARAMMAS-50GMS-POU', '09109100', 80, 763, 4807, 120, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 763, 'Tatvisha Enterprises', true, 0),
  ('Garam Masala [25 gms]', 'BM-GARAMMAS-25GMS-POU', '09109100', 41, 789, 3314, 160, 5, 'BLENDED SPICES', 'POUCH', 25.0, 'gms', 'CARTOON', 789, 'Tatvisha Enterprises', true, 0),
  ('Garam Masala Jar [100 gms]', 'BM-GARAMMAS-100GM-JAR', '09109100', 158, 753, 9488, 120, 5, 'BLENDED SPICES', 'JAR', 100.0, 'gms', 'CARTOON', 753, 'Tatvisha Enterprises', true, 0),
  ('Kitchen King (ACB) [100 gms]', 'BM-KITCHENK-100GM-ACB', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'ACB', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Kitchen King (ACB) [50 gms]', 'BM-KITCHENK-50GMS-ACB', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'ACB', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Amchur Powder [100 gms]', 'BM-AMCHURPO-100GM-POU', '08045090', 109, 520, 6552, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 520, 'Tatvisha Enterprises', true, 0),
  ('Amchur Powder [50 gms]', 'BM-AMCHURPO-50GMS-POU', '08045090', 55, 528, 4435, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 528, 'Tatvisha Enterprises', true, 0),
  ('Jaljira (ACB) [100 gms]', 'BM-JALJIRAA-100GM-ACB', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'ACB', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Jaljira (ACB) [50 gms]', 'BM-JALJIRAA-50GMS-ACB', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'ACB', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Punjabi Tadka Masala [100 gms]', 'BM-PUNJABIT-100GM-POU', '09109100', 103, 489, 6161, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 489, 'Tatvisha Enterprises', true, 0),
  ('Sambar Masala [100 gms]', 'BM-SAMBARMA-100GM-POU', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Sambar Masala [50 gms]', 'BM-SAMBARMA-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Fish Masala [100 gms]', 'BM-FISHMASA-100GM-POU', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Fish Masala [50 gms]', 'BM-FISHMASA-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Chana Masala [100 gms]', 'BM-CHANAMAS-100GM-POU', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Chana Masala [50 gms]', 'BM-CHANAMAS-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Chaat Masala [100 gms]', 'BM-CHAATMAS-100GM-POU', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Chaat Masala [50 gms]', 'BM-CHAATMAS-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Biriyani Masala [100 gms]', 'BM-BIRIYANI-100GM-POU', '09109100', 109, 520, 6552, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 520, 'Tatvisha Enterprises', true, 0),
  ('Biriyani Masala [50 gms]', 'BM-BIRIYANI-50GMS-POU', '09109100', 55, 528, 4435, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 528, 'Tatvisha Enterprises', true, 0),
  ('Manchurian Masala [100 gms]', 'BM-MANCHURI-100GM-POU', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Manchurian Masala [50 gms]', 'BM-MANCHURI-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('MoMo Masala [100 gms]', 'BM-MOMOMASA-100GM-POU', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('MoMo Masala [50 gms]', 'BM-MOMOMASA-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Chowmein Masala [100 gms]', 'BM-CHOWMEIN-100GM-POU', '09109100', 109, 520, 6552, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 520, 'Tatvisha Enterprises', true, 0),
  ('Chowmein Masala [50 gms]', 'BM-CHOWMEIN-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Pasta Masala [100 gms]', 'BM-PASTAMAS-100GM-POU', '09109100', 109, 520, 6552, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 520, 'Tatvisha Enterprises', true, 0),
  ('Pasta Masala [50 gms]', 'BM-PASTAMAS-50GMS-POU', '09109100', 55, 528, 4435, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 528, 'Tatvisha Enterprises', true, 0),
  ('Soya Chunks Masala [100 gms]', 'BM-SOYACHUN-100GM-POU', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Soya Chunks Masala [50 gms]', 'BM-SOYACHUN-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Egg Curry Masala [100 gms]', 'BM-EGGCURRY-100GM-POU', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Egg Curry Masala [50 gms]', 'BM-EGGCURRY-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Paneer Masala [100 gms]', 'BM-PANEERMA-100GM-POU', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Paneer Masala [50 gms]', 'BM-PANEERMA-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Pani Puri Masala [100 gms]', 'BM-PANIPURI-100GM-POU', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Pani Puri Masala [50 gms]', 'BM-PANIPURI-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Mushroom Curry Masala [100 gms]', 'BM-MUSHROOM-100GM-POU', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Mushroom Curry Masala [50 gms]', 'BM-MUSHROOM-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Dalma Masala [100 gms]', 'BM-DALMAMAS-100GM-POU', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Dalma Masala [50 gms]', 'BM-DALMAMAS-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Aloo Dum Masala [100 gms]', 'BM-ALOODUMM-100GM-POU', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Aloo Dum Masala [50 gms]', 'BM-ALOODUMM-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Pudina Chutney Powder [100 gms]', 'BM-PUDINACH-100GM-POU', '09109100', 97, 464, 5846, 120, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 464, 'Tatvisha Enterprises', true, 0),
  ('Pudina Chutney Powder [50 gms]', 'BM-PUDINACH-50GMS-POU', '09109100', 50, 474, 3982, 160, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 474, 'Tatvisha Enterprises', true, 0),
  ('Sabji Masala Pouch [200 gms]', 'BM-SABJIMAS-200GM-POU', '09109100', 188, 449, 3772, 40, 5, 'BLENDED SPICES', 'POUCH', 200.0, 'gms', 'CARTOON', 449, 'Tatvisha Enterprises', true, 0),
  ('Mix Masala Pouch [200 gms]', 'BM-MIXMASAL-200GM-POU', '09109100', 105, 250, 2100, 40, 5, 'BLENDED SPICES', 'POUCH', 200.0, 'gms', 'CARTOON', 250, 'Tatvisha Enterprises', true, 0),
  ('Kasuri Methi Pouch [250 gms]', 'BM-KASURIME-250GM-POU', '09109912', 178, 339, 1068, 12, 5, 'BLENDED SPICES', 'POUCH', 250.0, 'gms', 'CARTOON', 339, 'Tatvisha Enterprises', true, 0),
  ('Kasuri Methi (ACB) [100 gms]', 'BM-KASURIME-100GM-ACB', '09109912', 76, 360, 2268, 60, 5, 'BLENDED SPICES', 'ACB', 100.0, 'gms', 'CARTOON', 360, 'Tatvisha Enterprises', true, 0),
  ('Kasuri Methi Pouch [50 gms]', 'BM-KASURIME-50GMS-POU', '09109912', 45, 366, 2306, 120, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 366, 'Tatvisha Enterprises', true, 0),
  ('Kasuri Methi Pouch [25 gms]', 'BM-KASURIME-25GMS-POU', '09109912', 21, 371, 2337, 240, 5, 'BLENDED SPICES', 'POUCH', 25.0, 'gms', 'CARTOON', 371, 'Tatvisha Enterprises', true, 0),
  ('Asafoetida (Hing) [100 gms]', 'BM-ASAFOETI-100GM-POU', '13019013', 290, 1381, 4350, 30, 5, 'BLENDED SPICES', 'POUCH', 100.0, 'gms', 'CARTOON', 1381, 'Tatvisha Enterprises', true, 0),
  ('Asafoetida (Hing) [50 gms]', 'BM-ASAFOETI-50GMS-POU', '13019013', 147, 1400, 4410, 60, 5, 'BLENDED SPICES', 'POUCH', 50.0, 'gms', 'CARTOON', 1400, 'Tatvisha Enterprises', true, 0),
  ('Asafoetida (Hing) [25 gms]', 'BM-ASAFOETI-25GMS-POU', '13019013', 79, 1505, 4741, 120, 5, 'BLENDED SPICES', 'POUCH', 25.0, 'gms', 'CARTOON', 1505, 'Tatvisha Enterprises', true, 0),
  ('Asafoetida (Hing) [10 gms]', 'BM-ASAFOETI-10GMS-POU', '13019013', 36, 1734, 5462, 300, 5, 'BLENDED SPICES', 'POUCH', 10.0, 'gms', 'CARTOON', 1734, 'Tatvisha Enterprises', true, 0),
  ('Saffron [0.5 gms]', 'BM-SAFFRON0-05GMS-POU', '09102010', 250, 129, 2709, 20, 5, 'BLENDED SPICES', 'POUCH', 0.5, 'gms', 'CARTOON', 129, 'Tatvisha Enterprises', true, 0),
  ('Saffron [0.25 gms]', 'BM-SAFFRON0-025GM-POU', '09102010', 126, 90, 1890, 20, 5, 'BLENDED SPICES', 'POUCH', 0.25, 'gms', 'CARTOON', 90, 'Tatvisha Enterprises', true, 0),
  ('Cumin Seed (Jeera) [200 gms]', 'BM-CUMINSEE-200GM-POU', '09093129', 155, 369, 6199, 80, 5, 'WHOLE SPICES', 'POUCH', 200.0, 'gms', 'BAG', 369, 'Tatvisha Enterprises', true, 0),
  ('Cumin Seed (Jeera) [100 gms]', 'BM-CUMINSEE-100GM-POU', '09093129', 78, 370, 6216, 160, 5, 'WHOLE SPICES', 'POUCH', 100.0, 'gms', 'BAG', 370, 'Tatvisha Enterprises', true, 0),
  ('Cumin Seed (Jeera) [50 gms]', 'BM-CUMINSEE-50GMS-POU', '09093129', 39, 372, 6250, 320, 5, 'WHOLE SPICES', 'POUCH', 50.0, 'gms', 'BAG', 372, 'Tatvisha Enterprises', true, 0),
  ('Cumin Seed (Jeera) [1 kg]', 'BM-CUMINSEE-1KG-BAG', '09093129', 654, 340, 5440, 16, 5, 'WHOLE SPICES', 'BAG', 1.0, 'kg', 'BAG', 340, 'Tatvisha Enterprises', true, 0),
  ('Panch Phutan Odisha [500 gms]', 'BM-PANCHPHU-500GM-POU', '09109919', 119, 113, 1898, 32, 5, 'WHOLE SPICES', 'POUCH', 500.0, 'gms', 'BAG', 113, 'Tatvisha Enterprises', true, 0),
  ('Panch Phutan Odisha [250 gms]', 'BM-PANCHPHU-250GM-POU', '09109919', 61, 116, 1949, 64, 5, 'WHOLE SPICES', 'POUCH', 250.0, 'gms', 'BAG', 116, 'Tatvisha Enterprises', true, 0),
  ('Panch Phutan Odisha [100 gms]', 'BM-PANCHPHU-100GM-POU', '09109919', 25, 117, 1966, 160, 5, 'WHOLE SPICES', 'POUCH', 100.0, 'gms', 'BAG', 117, 'Tatvisha Enterprises', true, 0),
  ('Panch Phutan Odisha [50 gms]', 'BM-PANCHPHU-50GMS-POU', '09109919', 13, 119, 1999, 320, 5, 'WHOLE SPICES', 'POUCH', 50.0, 'gms', 'BAG', 119, 'Tatvisha Enterprises', true, 0),
  ('Mustard [250 gms]', 'BM-MUSTARD2-250GM-POU', '09109927', 58, 110, 1848, 64, 5, 'WHOLE SPICES', 'POUCH', 250.0, 'gms', 'BAG', 110, 'Tatvisha Enterprises', true, 0),
  ('Mustard [100 gms]', 'BM-MUSTARD1-100GM-POU', '09109927', 24, 112, 1882, 160, 5, 'WHOLE SPICES', 'POUCH', 100.0, 'gms', 'BAG', 112, 'Tatvisha Enterprises', true, 0),
  ('Mustard [50 gms]', 'BM-MUSTARD5-50GMS-POU', '09109927', 12, 114, 1915, 320, 5, 'WHOLE SPICES', 'POUCH', 50.0, 'gms', 'BAG', 114, 'Tatvisha Enterprises', true, 0),
  ('Mustard [1 kg]', 'BM-MUSTARD1-1KG-BAG', '09109927', 187, 97, 1552, 16, 5, 'WHOLE SPICES', 'BAG', 1.0, 'kg', 'BAG', 97, 'Tatvisha Enterprises', true, 0),
  ('Coriander Whole [100 gms]', 'BM-CORIANDE-100GM-POU', '09092190', 54, 258, 4334, 160, 5, 'WHOLE SPICES', 'POUCH', 100.0, 'gms', 'BAG', 258, 'Tatvisha Enterprises', true, 0),
  ('Coriander Whole [50 gms]', 'BM-CORIANDE-50GMS-POU', '09092190', 27, 260, 4368, 320, 5, 'WHOLE SPICES', 'POUCH', 50.0, 'gms', 'BAG', 260, 'Tatvisha Enterprises', true, 0),
  ('Fenugreek Seed (Methi) [100 gms]', 'BM-FENUGREE-100GM-POU', '09109912', 23, 110, 1848, 160, 5, 'WHOLE SPICES', 'POUCH', 100.0, 'gms', 'BAG', 110, 'Tatvisha Enterprises', true, 0),
  ('Fenugreek Seed (Methi) [1 kg]', 'BM-FENUGREE-1KG-BAG', '09109912', 192, 100, 1600, 16, 5, 'WHOLE SPICES', 'BAG', 1.0, 'kg', 'BAG', 100, 'Tatvisha Enterprises', true, 0),
  ('Black Cumin Seed (Kalajeera) [50 gms]', 'BM-BLACKCUM-50GMS-POU', '09093119', 32, 300, 5040, 320, 5, 'WHOLE SPICES', 'POUCH', 50.0, 'gms', 'BAG', 300, 'Tatvisha Enterprises', true, 0),
  ('Black Cumin Seed (Kalajeera) [500 gms]', 'BM-BLACKCUM-500GM-POU', '09093119', 264, 274, 4384, 32, 5, 'WHOLE SPICES', 'POUCH', 500.0, 'gms', 'BAG', 274, 'Tatvisha Enterprises', true, 0),
  ('Poppy Seed (Postak) [50 gms]', 'BM-POPPYSEE-50GMS-POU', '09109926', 179, 1700, 28560, 320, 5, 'WHOLE SPICES', 'POUCH', 50.0, 'gms', 'BAG', 1700, 'Tatvisha Enterprises', true, 0),
  ('Caraway Seed (Juani) [50 gms]', 'BM-CARAWAYS-50GMS-POU', '09109914', 28, 270, 4536, 320, 5, 'WHOLE SPICES', 'POUCH', 50.0, 'gms', 'BAG', 270, 'Tatvisha Enterprises', true, 0),
  ('Caraway Seed (Juani) [500 gms]', 'BM-CARAWAYS-500GM-POU', '09109914', 253, 250, 4000, 32, 5, 'WHOLE SPICES', 'POUCH', 500.0, 'gms', 'BAG', 250, 'Tatvisha Enterprises', true, 0),
  ('Fennel Seed (Madhuri) [50 gms]', 'BM-FENNELSE-50GMS-POU', '09096230', 26, 250, 4200, 320, 5, 'WHOLE SPICES', 'POUCH', 50.0, 'gms', 'BAG', 250, 'Tatvisha Enterprises', true, 0),
  ('Fennel Seed (Madhuri) [500 gms]', 'BM-FENNELSE-500GM-POU', '09096230', 221, 230, 3680, 32, 5, 'WHOLE SPICES', 'POUCH', 500.0, 'gms', 'BAG', 230, 'Tatvisha Enterprises', true, 0),
  ('Chilly Whole (Stick Less) [100 gms]', 'BM-CHILLYWH-100GM-POU', '', 74, 350, 1103, 30, 5, 'WHOLE SPICES', 'POUCH', 100.0, 'gms', 'BAG', 350, 'Tatvisha Enterprises', true, 0),
  ('Bay Leaf (Tej Patra) [50 gms]', 'BM-BAYLEAFT-50GMS-POU', '09109990', 17, 160, 1008, 120, 5, 'WHOLE SPICES', 'POUCH', 50.0, 'gms', 'BAG', 160, 'Tatvisha Enterprises', true, 0),
  ('Yellow Mustard [50 gms]', 'BM-YELLOWMU-50GMS-POU', '12075010', 14, 135, 2268, 320, 5, 'WHOLE SPICES', 'POUCH', 50.0, 'gms', 'BAG', 135, 'Tatvisha Enterprises', true, 0),
  ('Black Salt [100 gms]', 'BM-BLACKSAL-100GM-POU', '25010090', 10, 50, 1250, 250, 5, 'WHOLE SPICES', 'POUCH', 100.0, 'gms', 'BAG', 50, 'Tatvisha Enterprises', true, 0),
  ('Black Salt [1 kg]', 'BM-BLACKSAL-1KG-BAG', '25010090', 80, 40, 1000, 25, 5, 'WHOLE SPICES', 'BAG', 1.0, 'kg', 'BAG', 40, 'Tatvisha Enterprises', true, 0),
  ('Sandhrab Salt [100 gms]', 'BM-SANDHRAB-100GM-POU', '2501', 10, 52, 1300, 250, 5, 'WHOLE SPICES', 'POUCH', 100.0, 'gms', 'BAG', 52, 'Tatvisha Enterprises', true, 0),
  ('Sandhrab Salt [1 kg]', 'BM-SANDHRAB-1KG-BAG', '2501', 84, 42, 1050, 25, 5, 'WHOLE SPICES', 'BAG', 1.0, 'kg', 'BAG', 42, 'Tatvisha Enterprises', true, 0),
  ('Bio Soda [100 gms]', 'BM-BIOSODA1-100GM-POU', '28363000', 13, 54, 1593, 250, 5, 'WHOLE SPICES', 'POUCH', 100.0, 'gms', 'BAG', 54, 'Tatvisha Enterprises', true, 0),
  ('Papad Masala [200 gms]', 'BM-PAPADMAS-200GM-POU', '19059040', 81, 203, 2436, 60, 5, 'FOOD ITEMS', 'POUCH', 200.0, 'gms', 'BAG', 203, 'Tatvisha Enterprises', true, 0),
  ('Papad Masala [100 gms]', 'BM-PAPADMAS-100GM-POU', '19059040', 41, 205, 2460, 120, 5, 'FOOD ITEMS', 'POUCH', 100.0, 'gms', 'BAG', 205, 'Tatvisha Enterprises', true, 0),
  ('Sattu Channa [500 gms]', 'BM-SATTUCHA-500GM-POU', '21069099', 135, 129, 2167, 32, 5, 'FOOD ITEMS', 'POUCH', 500.0, 'gms', 'BAG', 129, 'Tatvisha Enterprises', true, 0),
  ('Sattu Channa [200 gms]', 'BM-SATTUCHA-200GM-POU', '21069099', 55, 131, 2201, 80, 5, 'FOOD ITEMS', 'POUCH', 200.0, 'gms', 'BAG', 131, 'Tatvisha Enterprises', true, 0),
  ('Sattu Channa [100 gms]', 'BM-SATTUCHA-100GM-POU', '21069099', 29, 138, 1159, 80, 5, 'FOOD ITEMS', 'POUCH', 100.0, 'gms', 'BAG', 138, 'Tatvisha Enterprises', true, 0),
  ('Multigrain Sattu [500 gms]', 'BM-MULTIGRA-500GM-POU', '21069099', 142, 110, 1848, 32, 5, 'FOOD ITEMS', 'POUCH', 500.0, 'gms', 'BAG', 110, 'Tatvisha Enterprises', true, 0),
  ('Panjabi Dal Tadka [500 gms]', 'BM-PANJABID-500GM-POU', '11041900', 137, 130, 2184, 32, 5, 'FOOD ITEMS', 'POUCH', 500.0, 'gms', 'BAG', 130, 'Tatvisha Enterprises', true, 0),
  ('Panjabi Dal Tadka [250 gms]', 'BM-PANJABID-250GM-POU', '11041900', 70, 132, 2218, 64, 5, 'FOOD ITEMS', 'POUCH', 250.0, 'gms', 'BAG', 132, 'Tatvisha Enterprises', true, 0),
  ('Sagoo Big [500 gms]', 'BM-SAGOOBIG-500GM-POU', '11062010', 101, 96, 1613, 32, 5, 'FOOD ITEMS', 'POUCH', 500.0, 'gms', 'BAG', 96, 'Tatvisha Enterprises', true, 0),
  ('Sagoo Small [500 gms]', 'BM-SAGOOSMA-500GM-POU', '11062010', 100, 95, 1596, 32, 5, 'FOOD ITEMS', 'POUCH', 500.0, 'gms', 'BAG', 95, 'Tatvisha Enterprises', true, 0),
  ('Sagoo Big [200 gms]', 'BM-SAGOOBIG-200GM-POU', '11062010', 41, 98, 1646, 80, 5, 'FOOD ITEMS', 'POUCH', 200.0, 'gms', 'BAG', 98, 'Tatvisha Enterprises', true, 0),
  ('Sagoo Small [200 gms]', 'BM-SAGOOSMA-200GM-POU', '11062010', 41, 97, 1630, 80, 5, 'FOOD ITEMS', 'POUCH', 200.0, 'gms', 'BAG', 97, 'Tatvisha Enterprises', true, 0),
  ('Wheat Dalia [500 gms]', 'BM-WHEATDAL-500GM-POU', '1101', 65, 62, 1042, 32, 5, 'FOOD ITEMS', 'POUCH', 500.0, 'gms', 'BAG', 62, 'Tatvisha Enterprises', true, 0),
  ('Wheat Dalia [200 gms]', 'BM-WHEATDAL-200GM-POU', '1101', 28, 68, 1142, 80, 5, 'FOOD ITEMS', 'POUCH', 200.0, 'gms', 'BAG', 68, 'Tatvisha Enterprises', true, 0),
  ('Sooji Kesari [500 gms]', 'BM-SOOJIKES-500GM-POU', '1102', 71, 68, 1142, 32, 5, 'FOOD ITEMS', 'POUCH', 500.0, 'gms', 'BAG', 68, 'Tatvisha Enterprises', true, 0),
  ('Corn Flour [100 gms]', 'BM-CORNFLOU-100GM-POU', '11081200', 15, 73, 920, 120, 5, 'FOOD ITEMS', 'POUCH', 100.0, 'gms', 'BAG', 73, 'Tatvisha Enterprises', true, 0),
  ('Corn Flour [1 kg]', 'BM-CORNFLOU-1KG-BAG', '11081200', 83, 43, 688, 16, 5, 'FOOD ITEMS', 'BAG', 1.0, 'kg', 'BAG', 43, 'Tatvisha Enterprises', true, 0),
  ('Soya Chunks (Soyabadi) [200 gms]', 'BM-SOYACHUN-200GM-POU', '19049000', 55, 131, 825, 30, 5, 'FOOD ITEMS', 'POUCH', 200.0, 'gms', 'BAG', 131, 'Tatvisha Enterprises', true, 0),
  ('Soya Chunks (Soyabadi) [1 kg]', 'BM-SOYACHUN-1KG-BAG', '19049000', 174, 87, 1041, 12, 5, 'FOOD ITEMS', 'BAG', 1.0, 'kg', 'BAG', 87, 'Tatvisha Enterprises', true, 0),
  ('Soya Chunks (Soyabadi) [5 kg]', 'BM-SOYACHUN-5KG-BAG', '19049000', 770, 80, 1600, 4, 5, 'FOOD ITEMS', 'BAG', 5.0, 'kg', 'BAG', 80, 'Tatvisha Enterprises', true, 0),
  ('Gram Dal Flour (Besan) [1 kg]', 'BM-GRAMDALF-1KG-BAG', '11061000', 200, 100, 1600, 16, 5, 'FOOD ITEMS', 'BAG', 1.0, 'kg', 'BAG', 100, 'Tatvisha Enterprises', true, 0),
  ('Gram Dal Flour (Besan) [500 gms]', 'BM-GRAMDALF-500GM-BAG', '11061000', 101, 101, 1616, 32, 5, 'FOOD ITEMS', 'BAG', 500.0, 'gms', 'BAG', 101, 'Tatvisha Enterprises', true, 0),
  ('Gram Dal Flour (Besan) [250 gms]', 'BM-GRAMDALF-250GM-BAG', '11061000', 51, 102, 1632, 64, 5, 'FOOD ITEMS', 'BAG', 250.0, 'gms', 'BAG', 102, 'Tatvisha Enterprises', true, 0),
  ('Wheat Flour (Sarabati Atta) [5 kg]', 'BM-WHEATFLO-5KG-BAG', '11081100', 440, 44, 1100, 5, 5, 'FOOD ITEMS', 'BAG', 5.0, 'kg', 'BAG', 44, 'Tatvisha Enterprises', true, 0),
  ('Wheat Flour (Sarabati Atta) [1 kg]', 'BM-WHEATFLO-1KG-BAG', '11081100', 88, 44, 1100, 25, 5, 'FOOD ITEMS', 'BAG', 1.0, 'kg', 'BAG', 44, 'Tatvisha Enterprises', true, 0),
  ('Honey Lemon Tea [250 gms]', 'BM-HONEYLEM-250GM-POU', '21012090', 125, 300, 3150, 40, 5, 'FOOD ITEMS', 'POUCH', 250.0, 'gms', 'BAG', 300, 'Tatvisha Enterprises', true, 0),
  ('Tomato Ketchup [100 gms]', 'BM-TOMATOKE-100GM-POU', '21032000', 15, 560, 627, 72, 12, 'KETCHUP', 'POUCH', 100.0, 'gms', 'CARTOON', 560, 'Tatvisha Enterprises', true, 0),
  ('Tomato Ketchup [250 gms]', 'BM-TOMATOKE-250GM-POU', '21032000', 30, 739, 828, 48, 12, 'KETCHUP', 'POUCH', 250.0, 'gms', 'CARTOON', 739, 'Tatvisha Enterprises', true, 0),
  ('Tomato Ketchup [500 gms]', 'BM-TOMATOKE-500GM-POU', '21032000', 58, 739, 828, 24, 12, 'KETCHUP', 'POUCH', 500.0, 'gms', 'CARTOON', 739, 'Tatvisha Enterprises', true, 0),
  ('Tomato Ketchup [1 kg]', 'BM-TOMATOKE-1KG-POU', '21032000', 115, 739, 828, 12, 12, 'KETCHUP', 'POUCH', 1.0, 'kg', 'CARTOON', 739, 'Tatvisha Enterprises', true, 0),
  ('Tomato Ketchup Bottle [1 kg]', 'BM-TOMATOKE-1KG-BOT', '21032000', 155, 996, 1116, 12, 12, 'KETCHUP', 'BOTTLE', 1.0, 'kg', 'CARTOON', 996, 'Tatvisha Enterprises', true, 0),
  ('Tomato Ketchup Bottle [200 gms]', 'BM-TOMATOKE-200GM-BOT', '21032000', 50, 946, 1059, 36, 12, 'KETCHUP', 'BOTTLE', 200.0, 'gms', 'CARTOON', 946, 'Tatvisha Enterprises', true, 0),
  ('Green Chilli Sauce [700 gms]', 'BM-GREENCHI-700GM-POU', '21039020', 115, 900, 1008, 15, 12, 'KETCHUP', 'POUCH', 700.0, 'gms', 'CARTOON', 900, 'Tatvisha Enterprises', true, 0),
  ('Green Chilli Sauce [200 gms]', 'BM-GREENCHI-200GM-POU', '21039020', 50, 964, 1080, 36, 12, 'KETCHUP', 'POUCH', 200.0, 'gms', 'CARTOON', 964, 'Tatvisha Enterprises', true, 0),
  ('Red Chilli Sauce [1 kg]', 'BM-REDCHILL-1KG-POU', '21039020', 160, 1029, 1152, 12, 12, 'KETCHUP', 'POUCH', 1.0, 'kg', 'CARTOON', 1029, 'Tatvisha Enterprises', true, 0),
  ('Red Chilli Sauce [200 gms]', 'BM-REDCHILL-200GM-POU', '21039020', 50, 964, 1080, 36, 12, 'KETCHUP', 'POUCH', 200.0, 'gms', 'CARTOON', 964, 'Tatvisha Enterprises', true, 0),
  ('Soya Sauce [1 kg]', 'BM-SOYASAUC-1KG-POU', '21031000', 160, 1029, 1152, 12, 12, 'KETCHUP', 'POUCH', 1.0, 'kg', 'CARTOON', 1029, 'Tatvisha Enterprises', true, 0),
  ('Soya Sauce [200 gms]', 'BM-SOYASAUC-200GM-POU', '21031000', 50, 964, 1080, 36, 12, 'KETCHUP', 'POUCH', 200.0, 'gms', 'CARTOON', 964, 'Tatvisha Enterprises', true, 0),
  ('Vermicelli [900 gms]', 'BM-VERMICEL-900GM-POU', '19021900', 115, 1099, 1154, 20, 12, 'PASTA ITEMS', 'POUCH', 900.0, 'gms', 'CARTOON', 1099, 'Tatvisha Enterprises', true, 0),
  ('Vermicelli [400 gms]', 'BM-VERMICEL-400GM-POU', '19021900', 52, 1108, 1163, 45, 12, 'PASTA ITEMS', 'POUCH', 400.0, 'gms', 'CARTOON', 1108, 'Tatvisha Enterprises', true, 0),
  ('Vermicelli [200 gms]', 'BM-VERMICEL-200GM-POU', '19021900', 26, 1118, 1174, 90, 12, 'PASTA ITEMS', 'POUCH', 200.0, 'gms', 'CARTOON', 1118, 'Tatvisha Enterprises', true, 0),
  ('Vermicelli [5 kg]', 'BM-VERMICEL-5KG-BAG', '19021900', 471, 960, 960, 4, 12, 'PASTA ITEMS', 'BAG', 5.0, 'kg', 'CARTOON', 960, 'Tatvisha Enterprises', true, 0),
  ('Vermicelli [20 kg]', 'BM-VERMICEL-20KG-BAG', '19021900', 1846, 940, 940, 1, 12, 'PASTA ITEMS', 'BAG', 20.0, 'kg', 'CARTOON', 940, 'Tatvisha Enterprises', true, 0),
  ('Macaroni / Fusilli / Penne [900 gms]', 'BM-MACARONI-900GM-POU', '19020000', 125, 1159, 1217, 20, 12, 'PASTA ITEMS', 'POUCH', 900.0, 'gms', 'CARTOON', 1159, 'Tatvisha Enterprises', true, 0),
  ('Macaroni / Fusilli / Penne [400 gms]', 'BM-MACARONI-400GM-POU', '19020000', 56, 1167, 1225, 45, 12, 'PASTA ITEMS', 'POUCH', 400.0, 'gms', 'CARTOON', 1167, 'Tatvisha Enterprises', true, 0),
  ('Macaroni / Fusilli / Penne [5 kg]', 'BM-MACARONI-5KG-BAG', '19020000', 534, 1040, 1040, 4, 12, 'PASTA ITEMS', 'BAG', 5.0, 'kg', 'CARTOON', 1040, 'Tatvisha Enterprises', true, 0),
  ('Rice Vermicelli / Star Pasta [1 kg]', 'BM-RICEVERM-1KG-POU', '19020000', 126, 1082, 1136, 18, 12, 'PASTA ITEMS', 'POUCH', 1.0, 'kg', 'CARTOON', 1082, 'Tatvisha Enterprises', true, 0),
  ('Rice Vermicelli [500 gms]', 'BM-RICEVERM-500GM-POU', '19020000', 65, 1091, 1146, 36, 12, 'PASTA ITEMS', 'POUCH', 500.0, 'gms', 'CARTOON', 1091, 'Tatvisha Enterprises', true, 0),
  ('Rice Vermicelli [5 kg]', 'BM-RICEVERM-5KG-BAG', '19020000', 529, 1040, 1040, 4, 12, 'PASTA ITEMS', 'BAG', 5.0, 'kg', 'CARTOON', 1040, 'Tatvisha Enterprises', true, 0),
  ('Kheermix [200 gms]', 'BM-KHEERMIX-200GM-POU', '19021900', 88, 42, 3175, 72, 12, 'PASTA ITEMS', 'POUCH', 200.0, 'gms', 'CARTOON', 42, 'Tatvisha Enterprises', true, 0),
  ('Turmeric [Re.1/-(100pc)]', 'BM-CHN-TURMERIC-RE1100pc', '09103030', 1, 76, 1596, 2000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 76, 'Tatvisha Enterprises', true, 0),
  ('Chilli [Re.1/-(100pc)]', 'BM-CHN-CHILLI-RE1100pc', '09103030', 1, 76, 1596, 2000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 76, 'Tatvisha Enterprises', true, 0),
  ('Panch Phutan [Re.1/-(100pc)]', 'BM-CHN-PANCHPHU-RE1100pc', '09103030', 1, 76, 1596, 2000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 76, 'Tatvisha Enterprises', true, 0),
  ('Bio Soda [Re.1/-(100pc)]', 'BM-CHN-BIOSODA-RE1100pc', '28363000', 1, 76, 1794, 2000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 76, 'Tatvisha Enterprises', true, 0),
  ('Meat Masala [Re.1/-(100pc) 2gms]', 'BM-CHN-MEATMASA-RE1100pc', '09109100', 1, 76, 1596, 2000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 76, 'Tatvisha Enterprises', true, 0),
  ('Curry Powder [Re.1/-(100pc) 2gms]', 'BM-CHN-CURRYPOW-RE1100pc', '09109100', 1, 76, 1596, 2000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 76, 'Tatvisha Enterprises', true, 0),
  ('Chicken Masala [Re.1/-(100pc) 2gms]', 'BM-CHN-CHICKENM-RE1100pc', '09109100', 1, 76, 1596, 2000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 76, 'Tatvisha Enterprises', true, 0),
  ('Jaljira [Re.1/-(100pc) 2gms]', 'BM-CHN-JALJIRA-RE1100pc', '09109100', 1, 76, 1596, 2000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 76, 'Tatvisha Enterprises', true, 0),
  ('Turmeric [Rs.2/-(50pc)]', 'BM-CHN-TURMERIC-RS250pc', '09103030', 2, 71, 1491, 1000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 71, 'Tatvisha Enterprises', true, 0),
  ('Chilli [Rs.2/-(50pc)]', 'BM-CHN-CHILLI-RS250pc', '09103030', 2, 71, 1491, 1000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 71, 'Tatvisha Enterprises', true, 0),
  ('Coriander Powder [Rs.2/-(50pc)]', 'BM-CHN-CORIANDE-RS250pc', '09103030', 2, 71, 1491, 1000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 71, 'Tatvisha Enterprises', true, 0),
  ('Cumin Powder [Rs.2/-(50pc)]', 'BM-CHN-CUMINPOW-RS250pc', '09103030', 2, 71, 1491, 1000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 71, 'Tatvisha Enterprises', true, 0),
  ('Meat Masala [Rs.2/-(50pc) 3gms]', 'BM-CHN-MEATMASA-RS250pc', '09109100', 2, 71, 1491, 1000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 71, 'Tatvisha Enterprises', true, 0),
  ('Curry Powder [Rs.2/-(50pc) 3gms]', 'BM-CHN-CURRYPOW-RS250pc', '09109100', 2, 71, 1491, 1000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 71, 'Tatvisha Enterprises', true, 0),
  ('Chicken Masala [Rs.2/-(50pc) 3gms]', 'BM-CHN-CHICKENM-RS250pc', '09109100', 2, 71, 1491, 1000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 71, 'Tatvisha Enterprises', true, 0),
  ('Garam Masala [Rs.2/-(50pc) 3gms]', 'BM-CHN-GARAMMAS-RS250pc', '09109100', 2, 71, 1491, 1000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 71, 'Tatvisha Enterprises', true, 0),
  ('Garam Masala [Rs.2/-(100pc) 2gms]', 'BM-CHN-GARAMMAS-RS2100pc', '09109100', 2, 140, 2940, 2000, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 140, 'Tatvisha Enterprises', true, 0),
  ('Turmeric [Rs.5/-(60pc)]', 'BM-CHN-TURMERIC-RS560pc', '09103030', 5, 157, 2473, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 157, 'Tatvisha Enterprises', true, 0),
  ('Chilli [Rs.5/-(60pc)]', 'BM-CHN-CHILLI-RS560pc', '09103030', 5, 157, 2473, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 157, 'Tatvisha Enterprises', true, 0),
  ('Coriander Powder [Rs.5/-(60pc)]', 'BM-CHN-CORIANDE-RS560pc', '09103030', 5, 157, 2473, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 157, 'Tatvisha Enterprises', true, 0),
  ('Cumin Powder [Rs.5/-(60pc)]', 'BM-CHN-CUMINPOW-RS560pc', '09103030', 5, 157, 2473, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 157, 'Tatvisha Enterprises', true, 0),
  ('Cumin Seed [Rs.5/-(60pc)]', 'BM-CHN-CUMINSEE-RS560pc', '09093200', 5, 200, 3150, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 200, 'Tatvisha Enterprises', true, 0),
  ('Panch Phutan [Rs.5/-(60pc)]', 'BM-CHN-PANCHPHU-RS560pc', '09093200', 5, 200, 3150, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 200, 'Tatvisha Enterprises', true, 0),
  ('Meat Masala [Rs.5/-(60pc)]', 'BM-CHN-MEATMASA-RS560pc', '09109100', 5, 170, 2678, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 170, 'Tatvisha Enterprises', true, 0),
  ('Curry Powder [Rs.5/-(60pc)]', 'BM-CHN-CURRYPOW-RS560pc', '09109100', 5, 170, 2678, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 170, 'Tatvisha Enterprises', true, 0),
  ('Chicken Masala [Rs.5/-(60pc)]', 'BM-CHN-CHICKENM-RS560pc', '09109100', 5, 170, 2678, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 170, 'Tatvisha Enterprises', true, 0),
  ('Garam Masala [Rs.5/-(60pc)]', 'BM-CHN-GARAMMAS-RS560pc', '09109100', 5, 170, 2678, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 170, 'Tatvisha Enterprises', true, 0),
  ('Chowmein Masala [Rs.5/-(60pc)]', 'BM-CHN-CHOWMEIN-RS560pc', '09109100', 5, 170, 2678, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 170, 'Tatvisha Enterprises', true, 0),
  ('Egg Curry Masala [Rs.5/-(60pc)]', 'BM-CHN-EGGCURRY-RS560pc', '09109100', 5, 170, 2678, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 170, 'Tatvisha Enterprises', true, 0),
  ('Fish Masala [Rs.5/-(60pc)]', 'BM-CHN-FISHMASA-RS560pc', '09109100', 5, 170, 2678, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 170, 'Tatvisha Enterprises', true, 0),
  ('Paneer Masala [Rs.5/-(60pc)]', 'BM-CHN-PANEERMA-RS560pc', '09109100', 5, 170, 2678, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 170, 'Tatvisha Enterprises', true, 0),
  ('Pasta Masala [Rs.5/-(60pc)]', 'BM-CHN-PASTAMAS-RS560pc', '09109100', 5, 170, 2678, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 170, 'Tatvisha Enterprises', true, 0),
  ('Aloo Dum Masala [Rs.5/-(60pc)]', 'BM-CHN-ALOODUMM-RS560pc', '09109100', 5, 170, 2678, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 170, 'Tatvisha Enterprises', true, 0),
  ('Sambar Masala [Rs.5/-(60pc)]', 'BM-CHN-SAMBARMA-RS560pc', '09109100', 5, 170, 2678, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 170, 'Tatvisha Enterprises', true, 0),
  ('Biriyani Masala [Rs.5/-(60pc)]', 'BM-CHN-BIRIYANI-RS560pc', '09109100', 5, 170, 2678, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 170, 'Tatvisha Enterprises', true, 0),
  ('Punjabi Tadka Masala [Rs.5/-(60pc)]', 'BM-CHN-PUNJABIT-RS560pc', '09109100', 5, 170, 2678, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 170, 'Tatvisha Enterprises', true, 0),
  ('Black Pepper Powder [Rs.5/-(60pc)]', 'BM-CHN-BLACKPEP-RS560pc', '09109100', 5, 170, 2678, 900, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 170, 'Tatvisha Enterprises', true, 0),
  ('Curry Powder [Rs.5/-(20pc ACB)]', 'BM-CHN-CURRYPOW-RS520pcA', '09109100', 5, 70, 1544, 420, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 70, 'Tatvisha Enterprises', true, 0),
  ('Meat Masala [Rs.5/-(20pc ACB)]', 'BM-CHN-MEATMASA-RS520pcA', '09109100', 5, 70, 1544, 420, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 70, 'Tatvisha Enterprises', true, 0),
  ('Chicken Masala [Rs.5/-(20pc ACB)]', 'BM-CHN-CHICKENM-RS520pcA', '09109100', 5, 70, 1544, 420, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 70, 'Tatvisha Enterprises', true, 0),
  ('Garam Masala [Rs.5/-(20pc ACB)]', 'BM-CHN-GARAMMAS-RS520pcA', '09109100', 5, 70, 1544, 420, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 70, 'Tatvisha Enterprises', true, 0),
  ('Sambar Masala [Rs.5/-(20pc ACB)]', 'BM-CHN-SAMBARMA-RS520pcA', '09109100', 5, 70, 1544, 420, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 70, 'Tatvisha Enterprises', true, 0),
  ('Kitchen King [Rs.5/-(20pc ACB)]', 'BM-CHN-KITCHENK-RS520pcA', '09109100', 5, 70, 1544, 420, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 70, 'Tatvisha Enterprises', true, 0),
  ('Curry Powder [Rs.10/-(20pc ACB)]', 'BM-CHN-CURRYPOW-RS1020pc', '09109100', 10, 128, 1344, 200, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 128, 'Tatvisha Enterprises', true, 0),
  ('Meat Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-MEATMASA-RS1020pc', '09109100', 10, 128, 1344, 200, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 128, 'Tatvisha Enterprises', true, 0),
  ('Chicken Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-CHICKENM-RS1020pc', '09109100', 10, 128, 1344, 200, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 128, 'Tatvisha Enterprises', true, 0),
  ('Garam Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-GARAMMAS-RS1020pc', '09109100', 10, 128, 1344, 200, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 128, 'Tatvisha Enterprises', true, 0),
  ('Black Pepper Powder [Rs.10/-(20pc ACB)]', 'BM-CHN-BLACKPEP-RS1020pc', '09109100', 10, 128, 1344, 200, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 128, 'Tatvisha Enterprises', true, 0),
  ('Egg Curry Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-EGGCURRY-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('MoMo Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-MOMOMASA-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Pav Bhaji Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-PAVBHAJI-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Dalma Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-DALMAMAS-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Fish Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-FISHMASA-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Amchur Powder [Rs.10/-(20pc ACB)]', 'BM-CHN-AMCHURPO-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Jaljira [Rs.10/-(20pc ACB)]', 'BM-CHN-JALJIRA-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Sabji Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-SABJIMAS-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Kashmiri Chilli [Rs.10/-(20pc ACB)]', 'BM-CHN-KASHMIRI-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Pani Puri Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-PANIPURI-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Mushroom Curry Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-MUSHROOM-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Chana Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-CHANAMAS-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Aloo Dum Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-ALOODUMM-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Biriyani Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-BIRIYANI-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Chaat Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-CHAATMAS-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Punjabi Tadka Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-PUNJABIT-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Paneer Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-PANEERMA-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Pudina Chutney Powder [Rs.10/-(20pc ACB)]', 'BM-CHN-PUDINACH-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Sambar Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-SAMBARMA-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Tomato Powder [Rs.10/-(20pc ACB)]', 'BM-CHN-TOMATOPO-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Garlic Powder [Rs.10/-(20pc ACB)]', 'BM-CHN-GARLICPO-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Ginger Powder [Rs.10/-(20pc ACB)]', 'BM-CHN-GINGERPO-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Pasta Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-PASTAMAS-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Chowmein Masala [Rs.10/-(20pc ACB)]', 'BM-CHN-CHOWMEIN-RS1020pc', '09109100', 10, 117, 2457, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 117, 'Tatvisha Enterprises', true, 0),
  ('Turmeric [Rs.10/-(20pc ACB)]', 'BM-CHN-TURMERIC-RS1020pc', '09103030', 10, 110, 2310, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 110, 'Tatvisha Enterprises', true, 0),
  ('Chilli [Rs.10/-(20pc ACB)]', 'BM-CHN-CHILLI-RS1020pc', '09103030', 10, 110, 2310, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 110, 'Tatvisha Enterprises', true, 0),
  ('Poppy Seed (Postak) [Rs.20/-(20pc ACB)]', 'BM-CHN-POPPYSEE-RS2020pc', '09109926', 20, 270, 5670, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 270, 'Tatvisha Enterprises', true, 0),
  ('Cardamom [Rs.10/-(20pc ACB)]', 'BM-CHN-CARDAMOM-RS1020pc', '09093129', 10, 135, 2835, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 135, 'Tatvisha Enterprises', true, 0),
  ('Laung (Clove) [Rs.10/-(20pc ACB)]', 'BM-CHN-LAUNGCLO-RS1020pc', '09093129', 10, 135, 2835, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 135, 'Tatvisha Enterprises', true, 0),
  ('Javitri (Mace) [Rs.10/-(20pc ACB)]', 'BM-CHN-JAVITRIM-RS1020pc', '09093129', 10, 135, 2835, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 135, 'Tatvisha Enterprises', true, 0),
  ('Jaiphal (Nutmeg) [Rs.10/-(20pc ACB)]', 'BM-CHN-JAIPHALN-RS1020pc', '09093129', 10, 135, 2835, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 135, 'Tatvisha Enterprises', true, 0),
  ('Cinnamon (Dalchin) [Rs.10/-(20pc ACB)]', 'BM-CHN-CINNAMON-RS1020pc', '09093129', 10, 135, 2835, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 135, 'Tatvisha Enterprises', true, 0),
  ('Poppy Seed (Postak) [Rs.10/-(20pc ACB)]', 'BM-CHN-POPPYSEE-RS1020pc', '09093129', 10, 135, 2835, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 135, 'Tatvisha Enterprises', true, 0),
  ('Star Aniseed [Rs.10/-(20pc ACB)]', 'BM-CHN-STARANIS-RS1020pc', '09093129', 10, 135, 2835, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 135, 'Tatvisha Enterprises', true, 0),
  ('Sahajeera [Rs.10/-(20pc ACB)]', 'BM-CHN-SAHAJEER-RS1020pc', '09093129', 10, 135, 2835, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 135, 'Tatvisha Enterprises', true, 0),
  ('Yellow Mustard [Rs.10/-(20pc ACB)]', 'BM-CHN-YELLOWMU-RS1020pc', '09093129', 10, 135, 2835, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 135, 'Tatvisha Enterprises', true, 0),
  ('Caraway Seed (Juani) [Rs.10/-(20pc ACB)]', 'BM-CHN-CARAWAYS-RS1020pc', '09093129', 10, 135, 2835, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 135, 'Tatvisha Enterprises', true, 0),
  ('Cumin Seed (Jeera) [Rs.10/-(20pc ACB)]', 'BM-CHN-CUMINSEE-RS1020pc', '09093129', 10, 135, 2835, 400, 5, 'PROCESS ITEMS', 'CHAIN', NULL, NULL, 'PKT', 135, 'Tatvisha Enterprises', true, 0)
...
-- =========================================================
-- PHASE 3: ADDITIONAL SAMPLE DATA (Warehouses, Shops & Profiles)
-- =========================================================


-- 1. Ensure a default warehouse exists
INSERT INTO public.warehouses (name, code, is_showroom)
VALUES 
  ('Main Warehouse', 'WH-MAIN', false),
  ('City Showroom', 'SR-CITY', true)
ON CONFLICT (code) DO NOTHING;

-- 2. Ensure some sample shops exist
INSERT INTO public.shops (name, shop_type, phone, address, is_active, credit_limit, balance)
VALUES 
  ('General Stores Inc', 'retailer', '9876543210', '123 Main St, Sector 1', true, 50000.00, 0.00),
  ('Mega Wholesalers', 'wholesaler', '9876543211', 'Industrial Area Node A', true, 200000.00, 0.00)
ON CONFLICT DO NOTHING;

-- 3. Ensure Owner is defined with role 'owner' once their auth is created
-- Triggers and security rules will automatically map public profiles for authenticated users.
